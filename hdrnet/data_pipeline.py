...
class MyHDRDataPipeline(DataPipeline):
	...
	def _produce_one_sample(self):
		...
		input_queue = random.choice(input_files)
		output_queue = output_files[input_files.index(input_queue)]
		
		im_input = iio.imread(input_queue,'HDR-FI')
		im_output = cv2.imread(output_queue)
		...
		input_wl = im_input.max()
		output_wl = 255.0
		
		#Convert to Lab
		rows, cols, depth = im_input.shape
		for i in range(rows):
			for j in range(cols):
				im_input[i,j,:] = cs.rgb2lab(im_input[i,j,:])
				im_output[i,j,:] = cs.rgb2lab(im_output[i,j,:])
		im_input[:,:,1:] = im_input[:,:,1:] + 128.0
		im_output[:,:,1:] = im_output[:,:,1:] + 128
		
		#~ im_input = im_input / (input_wl+1.0) 
		#~ im_output = im_output / output_wl
		
		#~ im_input = color.rgb2lab(im_input)
		#~ im_output = color.rgb2lab(im_output)
...
class MyNewHDRDataPipeline(DataPipeline):
	...
	def _produce_one_sample(self):
		...
		with tf.Session(config=conf) as sess:
			im_input = iio.imread(sess.run(input_queue),'HDR-FI')
			im_output = cv2.imread(sess.run(output_queue))
			im_output = np.flip(im_output, 2)
...
class NewHDRDataPipeline(DataPipeline):
	"""Pipeline to process HDR images
  ...
  """
	def _produce_one_sample(self):
		...
		for f1,f2 in zip(input_files, output_files):
			wr_data = {}
			im_input = iio.imread(f1, 'HDR-FI')
			im_output = np.flip(cv2.imread(f2), 2)
			...
			wr_data['image_input'] = im_input
			wr_data['image_output'] = im_output
			self._writer.write(wr_data)
		
		recordfile = self._writer._fname
		self._writer.close()

		self._reader = RecordReader([recordfile], shuffle=self.shuffle, num_epochs=self.num_epochs)
...
class RecordReader(object):
  ...
  def _parse_example(self, serialized):
    ...
    #~ dtype_feats = self._get_dtype_features()
    #~ for s in dtype_feats:
      #~ feats[s] = dtype_feats[s]
    for s in sz_feats:
      feats[s] = sz_feats[s]
    ...
    for i, f in enumerate(self.FEATURES):
      ...
      #~ data[f] = tf.decode_raw(sample[f], self.DTYPES[i], name='decode_{}'.format(f))
