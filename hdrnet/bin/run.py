...
sys.path.append('/home/lum1nus/tryhard/hdrnet-master')

import hdrnet.models as models
import hdrnet.utils as utils
import hdrnet.colors as cs
...
def get_input_list(path):
  regex = re.compile(".*.(png|jpeg|jpg|tif|tiff|hdr)")
...
def main(args):
  ...
  with tf.Session(config=config) as sess:
    ...
    for idx, input_path in enumerate(inputs):
      ...
      im_input = iio.imread(input_path, 'HDR-FI') #added by me
      ...
      #~ input_wl = np.percentile(bri_map, 98)
      #~ im_input = im_input/input_wl
      
      #Normalize and convert to Lab
      #~ im_input = im_input/(im_input.max()+1.0)
      ...
      if args.lowres_input is None:
		lowres_input = cv2.resize(im_input, (net_shape, net_shape))
      ...
      #~ #Convert output from Lab to RGB
      #~ out_[:,:,1:] = out_[:,:,1:] - 128
      #~ out_ = cv2.cvtColor(out_.astype(np.float32), cv2.COLOR_Lab2RGB)
      #~ out_ = out_*255
      
      #PRINT THE RESULT
      out_ = np.flip(out_, 2)
      cv2.imwrite(output_path, out_)
