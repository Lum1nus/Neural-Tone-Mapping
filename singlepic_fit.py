...
true_img = iio.imread('./reinhard05/anyhere/dani_belgium.png')
...
for i in range(rows):
    for j in range(cols//4, cols):
        if (i+j)%10==0:
            ...
            hist,bins = np.histogram(l_map[max(0,i-70):min(rows, i+70), max(cols//4, j-70):min(j+70, cols)].ravel(), 25, normed = True)
            descriptor = np.append(descriptor, hist)
...
model.add(keras.layers.Dense(200, input_dim=53, activation='relu'))
