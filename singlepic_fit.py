...
for i in range(rows):
    for j in range(cols):
        l_map[i,j] = 0.299*input_img.item(i,j,2) + 0.587*input_img.item(i,j,1) + 0.114*input_img.item(i,j,0)
...
for i in range(rows):
    for j in range(cols//4, cols):
        if (i+j)%3==0:
            y, ii, qq = cs.rgb_to_yiq(true_img.item(i,j,2), true_img.item(i,j,1), true_img.item(i,j,0)) #its actually the same with hdr
            GT = np.append(GT, y)
